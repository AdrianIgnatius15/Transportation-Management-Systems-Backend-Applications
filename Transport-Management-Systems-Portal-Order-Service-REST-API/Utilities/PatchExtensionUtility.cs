...
{
    public static void PatchFrom<TSource, TDestination>(this TDestination destination, TSource source)
    {
    ...
    private static void PatchObjects(object destination, object source)
    {
        ...
        foreach (PropertyInfo sourceProperty in sourceType.GetProperties(BindingFlags.Public | BindingFlags.Instance))
        {
            ...
            PropertyInfo? destProperty = destType.GetProperty(sourceProperty.Name, BindingFlags.Public | BindingFlags.Instance);
            ...
            if (destProperty.PropertyType != typeof(string) && destProperty.PropertyType.IsClass)
            {
